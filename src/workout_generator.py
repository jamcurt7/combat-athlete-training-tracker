...
def safe_main_lift_load(
    ...
    if training_max <= 0:
        ...
        # Hard cap progression data if it somehow became unsafe.
...
def accessory_rotation(index_seed: int, workout_modifier: str = "normal") -> list[dict[str, Any]]:
    ...
    if workout_modifier == "fun":
        return selected + [
            make_exercise("Bike", "conditioning", "recovery", 1, 8, 12, 0, 5.0, "Easy finisher. Keep it playful, not brutal.")
        ]

    if workout_modifier == "chaos":
        return selected + [
            make_exercise("Mobility Flow", "mobility", "recovery", 1, 5, 10, 0, 4.0, "Odd-object style cooldown: move creatively but safely.")
        ]
...
def green_day_workout(
    ...
    if focus in {"Posterior chain / grappling", "posterior_chain"}:
        exercises = [
            make_exercise("Trap Bar Deadlift", "hinge", "main_lift", 4, 3, 5, trap_load, 8.0, "Main posterior-chain strength. Smooth reps only."),
            make_exercise(pull_name, "vertical_pull", "main_lift", 4, 3, 6, pull_load, 8.0, "Added weight only. Grappling pull strength."),
        ...
        exercises = [
            make_exercise("Bench Press", "horizontal_push", "main_lift", 4, 4, 6, bench_load, 8.0, "Main press. No grinders."),
            make_exercise(pull_name, "vertical_pull", "main_lift", 4, 3, 6, pull_load, 8.0, "Added weight only. Prioritize clean reps."),
        ...
        exercises = [
            make_exercise("Trap Bar Deadlift", "hinge", "main_lift", 3, 3, 5, trap_load, 8.0, "Main hinge strength."),
            make_exercise("Bench Press", "horizontal_push", "main_lift", 3, 4, 6, bench_load, 8.0, "Main press."),
            make_exercise(pull_name, "vertical_pull", "main_lift", 3, 3, 6, pull_load, 8.0, "Added weight only. Main pull."),
...
def yellow_day_workout(
    ...
    weighted_pull_load = safe_main_lift_load(weighted_pull_name, "Yellow", DEFAULT_TRAINING_LOADS.get(weighted_pull_name, 20))
    ...
    if focus in {"Posterior chain / grappling", "posterior_chain"}:
        exercises = [
            ...
            make_exercise(body_pull_name, "vertical_pull", "accessory", 3, 5, 10, 0, 8.0, "Bodyweight pulling volume."),
        ...
        exercises = [
            ...
            make_exercise(weighted_pull_name, "vertical_pull", "main_lift", 3, 3, 6, weighted_pull_load, 7.5, "Added weight only. Main pull."),
...
def orange_day_workout(
    ...
    if focus in {"Upper emphasis", "upper_grip"}:
        exercises = [
            ...
            make_exercise(body_pull_name, "vertical_pull", "accessory", 3, 5, 8, 0, 7.0, "Easy pulling. No grinders."),
...
def accessory_day_workout(time_available: int, workout_modifier: str = "normal") -> list[dict[str, Any]]:
    ...
    exercises = [
        ...
        make_exercise(body_pull_name, "vertical_pull", "accessory", 3, 5, 10, 0, 7.5, "Vertical pull variation."),
